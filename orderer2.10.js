...
client.bind({
  ...
  const check=(value,state,array)=>{
    if(value==10 && state=='leader' ){
      ...
      if(orderer_parse.state =='dead'){ //애플리케이션의 값을 5개 받으면 
        ...
        .then (async (data)=>{ //동기로 사용하기 위해 ()함수 앞에 async를 붙ㅌ여주고
            ...
            if (dead_array == 0 && i.rejoin !='yes'){
              await check(i.value,orderer_parse.state,dead_array); //await를 check 함수 앞에 붙여줌 //이렇게하지않으면 파일에 트랜잭션을 저장하기전 장부를 읽어옴;;
...
&& orderer_parse.rejoin !='yes'&&orderer_parse.state != 'dead' &&orderer_parse.state !='rejoin'){
  orderer_parse.state =  'follower';
  console.log('change follwer',orderer_parse);
}
   if(i.candidate == 'orderer2'&& orderer_parse.state!='leader'&& orderer_parse.rejoin !='yes'&&orderer_parse.state !='rejoin'){
