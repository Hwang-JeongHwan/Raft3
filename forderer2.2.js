...
fs.writeFile('./ledger2.txt','start\n');
...
var orderer2 = '{"id":"orderer2","state":"follower","term":1} '; //JSON형식으로 데이터 저장 

var orderer_parse = JSON.parse(orderer2);
...
client.bind({
  ...
  const check=(value,state,array,rejoin)=>{
    if(value==10 && state=='leader'&&rejoin!='yes'&&rejoin!='dead' ){
      console.log(value,'dead!!!!\n\n\n\n\n\n\n\n')
      orderer_parse.state = 'dead'; // 오더러를 죽이고
      if(array==0){
        dead_array = fs1.readFileSync('ledger2.txt').toString().split("\n");
...
const interval = setInterval(() => {
  ...
  var orderer_check_candidate = mergeJSON.merge(orderer_parse,checkcommit_msg);
...
}, Random.getRandomInt(1000,2000));///1초~~2초 사이의 랜덤값으로 전송


client.on("listening", function () {
  var address = client.address();
  console.log("orderer2 listening " +
...
client.on('message', (msg, rinfo) => {
  console.log(`orderer2 got: ${msg} from ${rinfo.address}:${rinfo.port}`);
  ...
  if(orderer_parse.state=='rejoin' && i.rejoincopy=='send'){ //오더러의 상태가 rejoin이고 카피 메시지를 리더가 보냈으면
    ...
    fs.appendFile('./ledger2.txt',copy_ledger)
        .then(()=>{
            return fs.readFile('./ledger2.txt');
        })
        .then ((data)=>{ //동기로 사용하기 위해 ()함수 앞에 async를 붙ㅌ여주고
            console.log('ledger2:',data.toString());
  ...
  if(orderer_parse.rejoin == 'yes' &&i.copy!='ok' &&i.copy !='finish'){ 
    ...
    "leaderport":9002,"rejoincopy":"send"}`;
  ...
  if (i.id==='app'&& orderer_parse.rejoin !='yes' &&i.finish !='finish'){ 
    ...
    if(before_cnt != i.cnt){  //이전 카운트와 현재 들어온 메시지의 카운트가 다를경우만 장부에 저장
      ...
      if(orderer_parse.state =='dead'){ //애플리케이션의 값을 5개 받으면 
        ...
        ,"rejoin_state":"${orderer_parse.state}","port":9002,"rejoinstart":${start}}`;
        ...
        ledger = `{"id":"${i.id}","key":"${i.key}","value":${i.value},"logindex":${i.cnt},"term":${orderer_parse.term}}\n`
        before_cnt = i.cnt 
    
        fs.appendFile('./ledger2.txt',ledger)
        .then(()=>{
            return fs.readFile('./ledger2.txt');
        })
    ...
    if (orderer_parse.state == 'leader' && orderer_parse.rejoin !='yes' &&i.finish !='finish'){
      ...
      var leader = {"msg":"copy","id":"orderer2","state":"leader","term":orderer_parse.term,"port":9002 } ;//leader면 카피 메시지를 전송하기 위해 json형식으로 선언//간소화 가능할듯
      //orderer1.xx = xx이런식으로 
    ...
    if (orderer_parse.state == 'candidate'){
      ...
      var leader = {"msg":"copy","id":"orderer2","state":"candiate","term":orderer_parse.term,"port":9002 } ;//leader면 카피 메시지를 전송하기 위해 json형식으로 선언//간소화 가능할듯
      //orderer1.xx = xx이런식으로 
      var copymsg = mergeJSON.merge(i,leader);//트랜잭션과 합쳐서 전송
      console.log('copymsg',copymsg);
    ...
    //console.log('ledger = ',ledger);
      var ok = {"orderer_state":"ok","orderer_id":"2"};
      ok.time = Date.now();
   ...
   if(orderer_parse.state == 'candidate' && i.orderer_state =='ok'&& i.orderer_id != before_id){//ok메시지가 오고 orderer가 candidate 면
    console.log(before_id);
 ...
 // console.log('cmtime :',cmtime);
  ...
  if (before == i.cnt){ // before = 0을 초기값으로 갖는 이전값과 commitmsg.cnt를 비교 해서 같으면
    ...
    if(commitmsg.commit_cnt ==2 ){
      ...
      fs.copyFile('cmledger.txt','ledger2.txt')//cmledger의 장부를 먼저 복사해 오고 ledger의 값을 장부에 추가 왜냐하면 
...
if((i.candidate =='orderer1'||i.candidate =='orderer5'||i.candidate =='orderer3'||i.candidate =='orderer4')
&& orderer_parse.rejoin !='yes'&&orderer_parse.state != 'dead' &&orderer_parse.state !='rejoin'&&orderer_parse.state != 'favorite'){
  ...
  console.log('change follwer',orderer_parse);
   ...
   if(i.candidate == 'orderer2'&& orderer_parse.state!='leader'&& orderer_parse.rejoin !='yes'&&orderer_parse.state !='rejoin'&&orderer_parse.state != 'dead'){
    ...
    if(orderer_parse.state == 'leader' && i.commit == 'commit'  && orderer_parse.rejoin !='yes')
    {
      ...
      fs.copyFile('ledger2.txt','cmledger.txt')
              .then(()=>{
                  console.log('복사완료');
    ...
    {
      if((i.fav1 =='2' || i.fav2 == '2')&&orderer_parse.state != 'rejoin'&&
      ...
      if(i.fav1 !='2' && i.fav2 !='2'&&orderer_parse.state != 'rejoin'&&orderer_parse.state !='dead'&&orderer_parse.state != 'candidate'){
      ...
      console.log('this is commit');
      fs.copyFile('cmledger.txt','ledger2.txt')
              .then(()=>{
                  console.log('cmledger 복사완료');
    ...
    if(i.leader == 'noleader'){
        const timer = async () => {
          try{
            ...
            await _sleep(time); //1~2초 사이의 랜덤값이 지난후 전송하도록
            orderer_parse.term+=1;
            var vote = {};
            vote.id = orderer_parse.id;
            vote.votestate = orderer_parse.state;
            vote.term = orderer_parse.term;
            vote.leader = 'no leader';
            var vote_str = JSON.stringify(vote);
