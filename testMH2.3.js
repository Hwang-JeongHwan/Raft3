...
server.on('message', (msg, rinfo) => {
  ...
  if (8000<rinfo.port&& rinfo.port<9000){
    ...
    if(i.finish == 'finish'){ //finish를 보내 새로운 리더 선출 
      server.send(msg,9001,HOST,()=>{
        console.log('send finish',msg);
      });
      server.send(msg,9002,HOST,()=>{
        console.log('send finish',msg);
      });
      server.send(msg,9003,HOST,()=>{
        console.log('send finish',msg);
      });
      server.send(msg,9004,HOST,()=>{
        console.log('send finish',msg);
      });
      server.send(msg,9005,HOST,()=>{
        console.log('send finish',msg);
      });
...
/*function check_term(before_id_check_term,i) {
  if(before_id_check_term != i){
    ...
    if(before_id != i.id ){ //오더러중 리더가 없으면 리더가 없다는 메시지를 전송
        ...
        if (i.state != 'leader' && i.state != undefined){
            cnt+=1;
            console.log('cnt',cnt);
            if (cnt == 8){
              cnt = 0;
